...
metadata <- metadata_full %>% filter(ReGroup %in% c('Baseline', 'FollUp'))

# Define Time_point based on ReGroup
metadata$Time_point <- ifelse(metadata$ReGroup == 'Baseline', 'T0', 'T2') 
...
omics_long$Time_point <- factor(omics_long$Time_point, levels = c("T0", "T2"))
...
feature_deltas <- omics_long %>%
  ...
  filter(Time_point %in% c("T0", "T2")) %>%
  mutate(Time_point = factor(Time_point, levels = c("T0", "T2"))) %>%
  pivot_wider(names_from = Time_point, values_from = value) %>%
  filter(!is.na(T0) & !is.na(T2)) %>%
  mutate(delta_feature = T2 - T0)
...
p <- ggplot(cor_lodes,
  ...
  theme_minimal() +
  labs(
    title = "Top Omics–Symptom Correlations (3 months vs. Baseline)",
...
# Save output
# pdf("./Analysis/Analysis_Symptoms/Results/Top_Omics_Symptom_Correlations_T2_CLR.pdf", width = 16, height = 8)
# print(p)
# dev.off()
...
# Write to CSV
write.csv(cor_results_export, "./Analysis/Analysis_Symptoms/Results/Top_Omics_Symptom_Correlations_T2_CLR.csv", row.names = FALSE)